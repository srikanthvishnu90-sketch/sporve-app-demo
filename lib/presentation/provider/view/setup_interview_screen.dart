...
class _SetupInterviewScreenState extends State<SetupInterviewScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<SetupInterviewController>().start(
    ...
    switch (p.step) {
      ...
      case 1:
        return [
          _chip(
            '1-on-1 private',
            () => p.answerKinds(private: true, group: false),
          ),
          _chip(
            'Small groups',
            () => p.answerKinds(private: false, group: true),
          ),
          _chip(
            'Both',
            () => p.answerKinds(private: true, group: true),
            solid: true,
          ),
        ];
      case 2:
      ...
      case 5:
        return [_chip('Skip for now', p.skipLocation)];
      case 6:
        return [
          _chip(
            'Use the suggested policy',
            p.useTemplateCancellation,
            solid: true,
          ),
        ];
      case 7:
      ...
      child: Column(
        ...
        children: [
          ...
          _summaryRow(
            ...
            () => _editText('What to bring', p.whatToBring, p.setWhatToBring),
          ),
    ...
    children: [
      ...
      if (p.offersGroup)
        _servicePill(
          ...
          () => _editPrice('Group price', p.groupPriceCents, p.setGroupPrice),
        ),
    ],
  );
      ...
      child: Row(
        children: [
          ...
          GestureDetector(
            onTap: onEdit,
            behavior: HitTestBehavior.opaque,
            child: const Icon(Icons.edit_outlined, color: _topIcon, size: 16),
          ),
        ],
      ),
    ),
  ...
  void _editDuration(SetupInterviewController p) =>
      _pickSheet('Session length', [
        for (final m in [30, 45, 60, 90])
          _SheetOption('$m minutes', () => p.setDuration(m)),
      ]);

  void _editAvailability(
    SetupInterviewController p,
  ) => _pickSheet('Availability', [
    _SheetOption('Weekday evenings', () => p.setAvailabilityChoice('weekday')),
    _SheetOption('Weekend mornings', () => p.setAvailabilityChoice('weekend')),
