...
class CreateTrainerBottomSheet extends StatefulWidget {
  ...
  @override
  State<CreateTrainerBottomSheet> createState() => _CreateTrainerBottomSheetState();
}
...
class _CreateTrainerBottomSheetState extends State<CreateTrainerBottomSheet> {
  ...
  void initState() {
    ...
    _specialty = TextEditingController(text: (p['specialty'] ?? p['bio'] ?? '').toString());
    _photo = TextEditingController(text: (p['photo_url'] ?? '').toString());
    _price = TextEditingController(text: (p['price'] ?? '').toString());
    _duration = TextEditingController(text: (p['duration_min'] ?? '60').toString());
  ...
  Future<void> _save() async {
    if (_name.text.trim().isEmpty) {
      Get.snackbar('Trainer name', 'Please enter the trainer\'s name.',
    ...
    if (ok) {
      Get.back();
      Get.snackbar(_isEdit ? 'Trainer updated' : 'Trainer added',
  ...
  InputDecoration _dec(String label) => InputDecoration(
        labelText: label,
        labelStyle: AppTypography.font(color: AppColors.textSecondary, fontSize: 13),
        filled: true,
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              Text(_isEdit ? 'Edit trainer' : 'Add a trainer',
                  style: AppTypography.font(
                      fontSize: 20, fontWeight: FontWeight.w800,
                      color: AppColors.textPrimary)),
              const SizedBox(height: 16),
              TextField(controller: _name, decoration: _dec('Trainer name'),
                  style: const TextStyle(color: AppColors.textPrimary)),
              const SizedBox(height: 12),
              TextField(controller: _specialty, decoration: _dec('Specialty / short description'),
                  style: const TextStyle(color: AppColors.textPrimary)),
              const SizedBox(height: 12),
              TextField(controller: _photo, decoration: _dec('Photo URL (optional)'),
                  style: const TextStyle(color: AppColors.textPrimary)),
              const SizedBox(height: 12),
              Row(children: [
                Expanded(child: TextField(controller: _price,
                    keyboardType: TextInputType.number, decoration: _dec('Price / session (\$)'),
                    style: const TextStyle(color: AppColors.textPrimary))),
                const SizedBox(width: 12),
                Expanded(child: TextField(controller: _duration,
                    keyboardType: TextInputType.number, decoration: _dec('Duration (min)'),
                    style: const TextStyle(color: AppColors.textPrimary))),
              ]),
              ...
              Text('The cut you keep from this trainer\'s bookings.',
                  style: AppTypography.font(fontSize: 12, color: AppColors.textSecondary)),
              ...
              if (_commissionType == 'percent') ...[
                Row(children: [
                  Expanded(
                    child: Slider(
                      value: _percent, min: 0, max: 100, divisions: 100,
                ...
                SporveButton(
                  ...
                  onPressed: _saving
                      ...
                      : () async {
                          ...
                          final ok = await context.read<ProviderController>().deleteTrainer(id);
                          if (ok) Get.back();
                        },
                ),
              ],
