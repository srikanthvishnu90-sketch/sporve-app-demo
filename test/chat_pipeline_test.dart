...
import 'package:sporve_app/core/data/policies.dart';
...
Map<String, dynamic> svc({
  ...
  'providerId': {
    ...
    if (verified) 'background_check_completed_at': '2026-08-20T12:00:00Z',
  },
  'price': price,
...
void main() {
  group('ACCEPTANCE (prompt 2): "private lessons under \$75"', () {
    test(
      'returns ONLY eligible under-\$75 1-on-1; unverified never appears',
      () {
        final intent = QueryIntentParser.parse(
          'what companies have private lessons under \$75',
        );
        ...
        expect(
          names.contains('Everglade'),
          isFalse,
          reason: 'unverified excluded',
        );
        ...
        expect(
          rows.every((r) => r.sessionTypes.contains('one_on_one')),
          isTrue,
        );
  ...
  group('Hard safety gate', () {
    ...
    test(
      'identity verification alone never satisfies the background check',
      () {
        ...
        (identityOnly['providerId'] as Map<String, dynamic>).remove(
  ...
  group('Filters (prompt 3 regression)', () {
    ...
    test(
      'comparison: intent=compare still excludes the unverified provider',
      () {
  ...
  group('Grounded demo proposals (mock generate-proposals)', () {
    test('keeps only verified, sport-matched, in-budget listings', () {
      ...
      final names = ranked
          .map((p) => (p['providerId'] as Map)['businessName'])
          .toList();
      expect(
        names,
        ['Apex'],
    ...
    test('unverified is never proposed even when cheapest + top-rated', () {
      ...
      expect(ranked.map((p) => (p['providerId'] as Map)['businessName']), [
        'Verified',
      ]);
    });
    ...
    test('no eligible supply -> empty (honest no-supply, never padded)', () {
      ...
      expect(
        MockRepository.groundProposals(seed, sport: 'basketball'),
        isEmpty,
      );
    });
  });
  ...
  group('Answer grounding (mock, deterministic)', () {
    ...
    test(
      'empty rows → honest no-match + widening suggestion, no ids',
      () async {
