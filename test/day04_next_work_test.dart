...
void main() {
  ...
  group('Day 04 Next Work Verification', () {
    test(
      '1. No Geographic Assumptions: Distance calculation works across 3 metros',
      () {
        final sampleService = {
          '_id': 'prog_nyc',
    ...
    test(
      '3. Kill Offline Demo in Prod: USE_MOCK_REPO is force-false in release mode',
      () {
        ...
        if (kReleaseMode) {
          expect(
            effectiveMockMode,
            isFalse,
            reason: 'Mock mode MUST be false in release mode',
          );
        } else {
          expect(
            effectiveMockMode,
            isTrue,
            reason: 'Debug mode allows mock mode for local testing',
          );
        }
      },
    );
    ...
    test(
      '4. Edge Function Invocation Check: Every client function name is non-empty',
      () {
