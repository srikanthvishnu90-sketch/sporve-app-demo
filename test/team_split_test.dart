...
void main() {
  ...
  group('TeamBlockPricing reuses platform_fee.dart (intro rate per family)', () {
    test('split_pay: each share is an intro-rate line, shares sum to block total', () {
      ...
      // Each family: $150 share at the 18% intro rate = $27 fee.
      expect(p.shares.first.shareCents, 15000);
      expect(p.shares.first.item.feeBps, kFirstBookingFeeBps);
      expect(p.shares.first.item.feeCents, 2700);
      // Aggregate fee across the roster = 4 x $27 = $108.
      expect(p.feeTotals.feeCents, 10800);
    });

    test('one_payer: a single whole-block intro line', () {
      final p = TeamBlockPricing.compute(
        sessionCount: 10,
        unitPriceCents: 5000,
      ...
      expect(p.shares.single.item.feeBps, kFirstBookingFeeBps);
      // Fee mirrors feeCentsFor exactly (no separate constant).
      expect(p.shares.single.item.feeCents, feeCentsFor(50000, kFirstBookingFeeBps));
    });
  });
  ...
  group('redeemSplitShare — the dozen-parents growth mechanic (mock)', () {
    ...
    test('a paid share provisions a consent-gated parent/athlete + N bookings', () async {
      ...
      final shareSum = status.fold<int>(
          0, (a, m) => a + (m['shareAmountCents'] as int));
      ...
      final kids = MockRepository.teamBlockAthletesForTest
          .where((a) => a['firstName'] == 'Mia');
    ...
    test('COPPA: an empty consent_version redeems NOTHING (gate, not bypass)', () async {
    ...
    test('a used link cannot be redeemed twice (idempotent no double-book)', () async {
...
String _tokenForFirstMember(String block) {
  final link = MockRepository.splitPayLinksForTest
      .firstWhere((l) => l['teamBlockId'] == block && l['status'] == 'pending');
